...
//--------------------------------------------------------------
void ofApp::setup(){
    ofDisableArbTex();      // テスクチャ座標を0 〜 1で扱う
    ofEnableSmoothing();
...
//    _filters.push_back(charcoal);
    ...
    _filters.push_back(new TiltShiftFilter(_video.getTexture()));
    _filters.push_back(new VoronoiFilter(_video.getTexture()));
    ...
    _filters.push_back(new HarrisCornerDetectionFilter(_video.getTexture()));
    _filters.push_back(new MotionDetectionFilter(_video.getTexture()));
...
//    _filters.push_back(new PoissonBlendFilter(wes.getTexture(), _video.getWidth(), _video.getHeight(), 2.0));
